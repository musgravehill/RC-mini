...
//#define POLLING_ENABLED  - закоменнтируй, нам не нужен такой поллинг
...
const byte GPS_buff_len = 90;
char GPS_CRCbuffer[GPS_buff_len];
...
void loop() {
  ...
  if (currentTime > displayTime)
  {
    ...
    float GPS_x = 10.1;
    float GPS_y = 20;
    String GPS_cmd = "$XY";    // a command name
    dtostrf(GPS_x, 4, 1, GPS_strX);   // format float value to string XX.X
    dtostrf(GPS_y, 4, 1, GPS_strY);
    GPS_msg = GPS_cmd + GPS_delim + GPS_strX + GPS_delim + GPS_strY + GPS_splat;
    GPS_outputMsg(GPS_msg); // print the entire message string, and append the CRC
